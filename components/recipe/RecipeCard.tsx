...
export default function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  const getSkillBadge = (level: string) => {
    ...
    return (
      <div className="flex items-center">
        <div className={`${styles[level].bg} w-6 h-6 rounded-full flex items-center justify-center text-white mr-2`}>
          {styles[level].text}
        </div>
  ...
  const getAllergyInfo = (recipe: Recipe) => {
    if (recipe.allergyInfo && recipe.allergyInfo.warnings && 
    ...
    } else if (recipe.allergyInfo && recipe.allergyInfo.safe && 
    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-4 mb-3">
          <div className="flex items-center">
            {getSkillBadge(recipe.skillLevel)}
          </div>
          <div className="flex items-center">
        ...
        {/* Make sure Link is working correctly */}
        <div className="flex justify-end">
          <Link 
            href={`/recipe/${recipe.id}`}
            className="px-4 py-2 bg-mise-500 text-white rounded-full text-sm hover:bg-mise-600"
            prefetch={false} // Add this to prevent automatic prefetching
          >
            View Recipe
          </Link>
        </div>
